"""Demand-weighted K-Means baseline for AP placement evaluation."""
...
def run_weighted_kmeans_baseline(
    ray_pool: "RawRayActorPoolExecutor",
    num_aps: int,
    fixed_z: float,
    ...
    """Run demand-weighted K-Means baseline and return standardized output.
    ...
    if spatial_weights is not None:
        ...
        if weights.shape[0] != coords.shape[0]:
            raise ValueError(
                "spatial_weights length must match floorplan_coords length "
                f"({weights.shape[0]} != {coords.shape[0]})"
            )
    ...
    if not results:
        raise RuntimeError("Weighted K-Means baseline received no worker results.")

    first_result = results[0]
    if not isinstance(first_result, Mapping):
        raise RuntimeError("Weighted K-Means baseline received an invalid worker result payload.")
    ...
    formatted = format_baseline_result(
        algorithm_name="weighted_kmeans",
...
__all__ = ["run_weighted_kmeans_baseline"]
