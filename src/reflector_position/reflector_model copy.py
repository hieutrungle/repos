...
class ReflectorController:
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    The reflector's centre is constrained to a planar wall patch defined by

    * ``wall_origin``  — one corner of the allowed rectangle,
    * ``wall_u_axis``  — lateral (horizontal) basis vector,
    * ``wall_v_axis``  — vertical basis vector.
    ...
    wall_origin : array-like, shape (3,)
        Corner of the wall placement rectangle (metres).
    wall_u_axis : array-like, shape (3,)
        Lateral basis vector spanning the wall rectangle.
    wall_v_axis : array-like, shape (3,)
        Vertical basis vector spanning the wall rectangle.
    ...
    def __init__(
        self,
        reflector: sionna.rt.SceneObject,
        wall_origin: Optional[np.ndarray] = None,
        wall_u_axis: Optional[np.ndarray] = None,
        wall_v_axis: Optional[np.ndarray] = None,
        ...
        if wall_origin is not None:
            self.set_wall_surface(wall_origin, wall_u_axis, wall_v_axis)
    ...
    def set_wall_surface(
        self,
        origin: np.ndarray | torch.Tensor,
        u_axis: np.ndarray | torch.Tensor,
        v_axis: np.ndarray | torch.Tensor,
    ) -> None:
        """Define the rectangular wall patch for placement.

        Parameters
        ----------
        origin : array-like, shape (3,)
            Corner of the rectangle (metres).
        u_axis : array-like, shape (3,)
            Full-length lateral vector (point = origin + u * u_axis).
        v_axis : array-like, shape (3,)
            Full-length vertical vector (point = origin + v * v_axis).
        """
    ...
    def wall_position(self) -> torch.Tensor:
        ...
        if self._wall_origin is None:
            raise RuntimeError(
                "Wall surface not configured. Call set_wall_surface() first."
            )
